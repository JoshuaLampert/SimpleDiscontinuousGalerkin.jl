...
function create_cache(mesh::OversetGridMesh, equations, solver)
    ...
    x_ll_L_ref = SummationByPartsOperators.xmin(D)
    x_ll_R_ref = SummationByPartsOperators.xmax(D)
    b_mapped = linear_map(b, xll_L, xll_R, x_ll_L_ref, x_ll_R_ref)
    ...
    x_lr_L_ref = SummationByPartsOperators.xmin(D)
    x_lr_R_ref = SummationByPartsOperators.xmax(D)
    c_mapped = linear_map(c, xlr_L, xlr_R, x_lr_L_ref, x_lr_R_ref)
...
function calc_boundary_flux!(surface_flux_values, u, t, boundary_conditions,
    ...
    e_right_L = get_projection_operator(cache_left.e_right, solver_left, nelements(mesh_left))
    ...
    e_right_R = get_projection_operator(cache_right.e_right, solver_right, nelements(mesh_right))
